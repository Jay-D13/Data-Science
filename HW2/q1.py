...
def convert_id(ID: str) -> str:
    ...
    """
    # Pour optimiser il faudrait faire un cache des résultats en les mettant dans une hashmap
    # puis pour eviter de lire le fichier à chaque fois, on pourrait le lire une seule fois
...
def convert_ids(labels: str) -> str:
    ...
    """
    result = [convert_id(ID) for ID in labels.split(",")]        
    return "|".join(result)
...
def get_conditional_proportion(labels: pd.Series, label_1: str, label_2: str) -> float:
    ...
    return label_2_series.shape[0] / label_1_series.shape[0]

    # ou vu que les deux séries sont de dimension 1: 
...
if __name__ == "__main__":
    ...
    series = pd.Series([
        ...
        "Voice|Speech",
        "Music|Piano",
        "Musical instrument|Theatre|Piano",
        "Music|Piano|Speech"
    ])
