...
def get_model_list():
    """
    Renvoyer la liste des modèles valides
    """
    return requests.get(f"{BASE_URL}/models")

def load_model():
    """
    Charger un modèle particulier sur le backend.
    """
    with st.spinner("Chargement en cours..."):
        return requests.put(f"{BASE_URL}/model/{st.session_state.model_name}")
...
with st.container():
    ...
    st.selectbox("Sélectionnez le modèle à utiliser", valid_models, key="model_name", on_change=load_model)
