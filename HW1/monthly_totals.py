...
def pivot_months_pandas(data: pd.DataFrame) -> Tuple[pd.DataFrame, pd.DataFrame]:
    ...
    décimale.
    """
    months = data["date"].apply(date_to_month)
    
    monthly = data.groupby(["name", months])["precipitation"].sum().reset_index()
    monthly = monthly.pivot(index="name", columns="date", values="precipitation")
    
    counts = data.groupby(["name", months])["precipitation"].count().reset_index()
    counts = counts.pivot(index="name", columns="date", values="precipitation")
...
# new_df = pd.DataFrame(squareform(pdist(df,metric=func)), index=df.index, columns=df.index)
...
def compute_pairwise(df: pd.DataFrame, func: callable) -> pd.DataFrame:
    ...
    **chaque paire de lignes du dataframe**.
    ...
    ```
    """
    distances = pdist(df, func)
    new_df = pd.DataFrame(squareform(distances), index=df.index, columns=df.index)
...
def compute_pairwise_distances(df: pd.DataFrame) -> pd.DataFrame:
    ...
    Utilisez compute_pairwise() avec la méthode geodesic() déjà complétée pour implémenter compute_pairwise_distance(), qui renverra une matrice des distances par paires entre les stations. Le point ici est de s'assurer que vous pivotez correctement les données.
    """
    
    df = df.drop_duplicates(subset=['name', 'latitude', 'longitude'])
    new_df = df.set_index('name')[['latitude', 'longitude']]
    new_df = new_df.sort_index() # cause gradescope has it ordered for some reason
...
def correlation(u, v) -> float:
    ...
    corr = ((u_valid - mean_u) * (v_valid - mean_v)).mean() / (std_u * std_v)
...
def main():
    ...
    data = get_precip_data(data_path + "precipitation.csv")
