...
def parse_xml(path: str) -> pd.DataFrame:
    ...
    data = []
    for person in root:
        attributes = {attribute: _get_val(person, attribute) for attribute in TARGET_ATTRIBUTES}
...
def clean_data(df: pd.DataFrame, year_cutoff: int) -> pd.DataFrame:
    ...
    df = df[df['birthday'].dt.year <= year_cutoff]
...
def bin_counts(df: pd.DataFrame, bins: list, bin_centers: list) -> pd.DataFrame:
    ...
    Arguments :
        ...
        bins (liste) : définit les bords des bacs ; c'est-à-dire que [0, 5, 10] définit deux classes : [0, 5) et [5, 10]
    ...
    binned = pd.cut(df['rating'], bins=bins, labels=bin_centers)
    hist = binned.value_counts().reset_index()
...
class PermutationTest:
    ...
    def job(self, seed: int = None) -> Tuple[int, int]:
        ...
        perm1 = permutation[:self.n_overrep]
        perm2 = permutation[self.n_overrep:self.n_overrep+self.n_underrep]
        return np.max(perm1), np.max(perm2)
